...
if uploaded_file:
    ...
    if not all(col in df.columns for col in required_cols):
        st.error("Missing required columns. Required: order-status, quantity, promotion-ids")
        st.stop()
    ...
    # Vine classification based on 'promotion-ids' column
    df['vine_order'] = df['promotion-ids'].str.contains('vine.enrollment', case=False, na=False)
    ...
    # Order counts
    total_orders = len(df)
    vine_orders = df['vine_order'].sum()
    retail_orders = total_orders - vine_orders
    pending_orders = (df['order-status'] == 'pending').sum()
